...
// Regression fixture from rapidsai/cudf#22777, generated by the Velox
// Parquet writer.
TEST_F(TopNTest, readsEmptyDictionaryPage) {
  const auto path = test::getDataFilePath(
      "velox/experimental/cudf/tests",
      "examples/one_null_row_dict.parquet");
...
INSTANTIATE_TEST_SUITE_P(
    TwoThreeFiveAndSixtyFiveRuns,
    TopNRowNumberRunTest,
    testing::Values(2, 3, 5, 65),
  ...
  for (int32_t run = 0; run < kRuns; ++run) {
    auto partition = makeFlatVector<int64_t>(
        kRowsPerRun, [run](vector_size_t row) {
          ...
          // candidate reduction is intentionally close to a no-op.
          return row == 0
              ? -1
